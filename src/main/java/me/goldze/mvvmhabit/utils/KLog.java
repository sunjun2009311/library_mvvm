...
public class KLog {
    ...
    public static void printLongStr(String TAG,String content) {
        int startPos = 0;
        int endPos = Math.min(LOG_MAXLENGTH, content.length());
        String current;
        while (true) {
            current = content.substring(startPos, endPos);
            startPos = endPos;
            int surplusLen = content.length() -endPos;//current.length();//startPos;
            endPos = startPos+Math.min(LOG_MAXLENGTH, surplusLen);
...
//            System.out.println(TAG+":"+current);
            Log.d(TAG, current);
            if(current.length()<LOG_MAXLENGTH||endPos==current.length()||startPos==endPos){
                break;
            }
            //不用递归了
        }
